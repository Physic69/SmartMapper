...
import com.example.try1.databinding.ActivityMainBinding // Import View Binding class
...
class MainActivity : AppCompatActivity() {
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                MotionSensorService.ACTION_SENSOR_DATA_UPDATE -> {
                    ...
                    // Display the live variance value from the binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pathViewModel.pathPoints.observe(this) { points ->
            binding.pathView.setPath(points)
        }

        binding.startButton.setOnClickListener {
            if (hasPermissions()) startMotionService() else requestPermissions()
        }
